...
urlpatterns = patterns(
    ...
    url(r'^shelter_call_callback/(?P<client_call>\d+)/(?P<pks>.*)/$', send_client_to_post, name='send_client_to_post'),
    url(r'^post_shelter_call/(?P<client_call>\d+)/(?P<pks>.*)/$', PostShelterCallView.as_view(), name='post_shelter_call'),
    url(r'^verify_shelter_availability/(?P<client_call>\d+)/$', VerifyShelterAvailabilityView.as_view(), name='verify_shelter_availability'),
)
